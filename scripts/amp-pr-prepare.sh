...
get_reviewers() {
    local reviewers=""
    # Add team leads based on module changes
    # Customize these based on your team structure
    if echo "$CHANGED_FILES" | grep -q "punchout-gateway"; then
...
# Use Sourcegraph AMP SDK for intelligent PR description (if available)
generate_amp_description() {
    # Check if AMP description was generated by the workflow
...
EOF
    ...
    # Try to get AMP-generated description
...
<!-- Add any deployment considerations here -->
...
main() {
    ...
    # Log the action
    cat >> amp-pr-prepare-log.txt << EOF
